...
Сводка, capacity dashboard, role workload charts, plan insights, **AI пульс спринта** (flow pace, iGaming Rip), **AI-панель**, секции отчёта, priority queues, activity feed, таблицы snapshot, режим презентации, печать в PDF.

**AI пульс спринта** — шесть донатов (done mix, throughput, cycle time, время в статусах, QA-итерации, активные сигналы). Детализация «Время в статусах»: задачи от длинных к коротким, внутри — timeline по Jira changelog.
